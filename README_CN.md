...
如果笔记中存在失效的图片链接（例如在不同编辑器间移动文件或更换操作系统导致），修复工具直接集成在无主图片清理面板中。

### 使用方法

1. 打开无主图片清理面板（侧边栏图标或命令面板）
2. 点击面板左下角的 **"修复当前笔记的失效链接"** 按钮
3. 插件自动修复当前笔记中的失效链接，并重新扫描
...
修复完成后，孤儿列表会自动刷新 -- 那些仅因链接失效而显示为"孤儿"的图片将从列表中消失。

### 定位按钮

每个孤儿图片右侧有两个操作按钮：

- **打开关联笔记**（文件图标）：通过 `.assets` 文件夹名模糊匹配找到对应的 markdown 笔记并打开，方便在清理前确认。
- **在文件资源管理器中打开**（文件夹图标）：在系统文件资源管理器中打开 `.assets` 文件夹，方便浏览同目录下的相关图片。
